...
from app.services.reporting import write_csv_report, write_html_report, write_json_report
...
def _to_report_payload(case: Case, profiles: list[Profile], findings: list[dict], risk: RiskScore) -> dict:
    return {
        "case_name": case.name,
...
async def search(req: SearchRequest, db: Session = Depends(get_db)) -> SearchResponse:
    ...
    profiles_out: list[Profile] = []
    timeline_out: list[TimelineEvent] = []
    findings: list[dict] = []
    ...
    for ident in req.identifiers:
        ...
        for w in await search_public_web(ident.value, limit=8):
            if w["url"] in seen_urls:
                continue
            seen_urls.add(w["url"])
            web_results.append(w)
    ...
    for web in web_results:
        db.add(Evidence(case_id=case.id, category="web_result", source=web.get("source", "web"), data=web))
    ...
    report_payload = _to_report_payload(case, profiles_out, findings, risk)
...
@router.get("/reports/{case_id}/export", response_model=ExportResponse)
def export_report(case_id: int, format: str = Query("html", pattern="^(html|json|csv)$"), db: Session = Depends(get_db)) -> ExportResponse:
    ...
    findings = [
        e.data for e in db.query(Evidence).filter(Evidence.case_id == case_id, Evidence.category == "finding").all()
    ]
    risk = db.query(RiskScore).filter(RiskScore.case_id == case_id).order_by(RiskScore.generated_at.desc()).first()
    if not risk:
        raise HTTPException(status_code=404, detail="Risk score not found")

    payload = _to_report_payload(case, profiles, findings, risk)
    output_path = f"reports/case_{case_id}.{format}"
    if format == "html":
        final_path = write_html_report(output_path, payload)
    elif format == "json":
        final_path = write_json_report(output_path, payload)
    else:
        final_path = write_csv_report(output_path, payload)

    return ExportResponse(case_id=case_id, format=format, path=final_path)
...
def dashboard_summary(db: Session = Depends(get_db)) -> DashboardSummary:
    ...
    recent = db.query(Case).order_by(Case.created_at.desc()).limit(5).all()
    high_findings = db.query(Evidence).filter(Evidence.category == "finding").order_by(Evidence.created_at.desc()).limit(5).all()
...
def get_case(case_id: int, db: Session = Depends(get_db)) -> dict:
    ...
    web_results = [
        e.data for e in db.query(Evidence).filter(Evidence.case_id == case_id, Evidence.category == "web_result").all()
    ]
